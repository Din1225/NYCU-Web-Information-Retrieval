# 主要用途：使用 Qwen/Qwen3-Embedding-4B 建立 dense retriever，文件端使用 Question + Answer。
...
def build_dense_cache_key(
    ...
    for document in documents:
        ...
        digest.update(b"\0")
        digest.update(normalize_text(document_text(document)).encode("utf-8"))
        digest.update(b"\0")
...
class DenseRetriever:
    """Dense retriever using Qwen/Qwen3-Embedding-4B on Question + Answer text."""
    ...
    def _load_or_build_embeddings(self) -> np.ndarray:
        ...
        texts = [normalize_text(document_text(document)) for document in self.documents]
