# 主要用途：負責讀取資料集、讀取 query 檔案，以及將檢索結果輸出成 JSON。
...
# 讀取指定路徑的 UTF-8 JSON 檔，並回傳解析後的 Python 物件。
...
# 將 Python 物件輸出成格式化 JSON，必要時自動建立輸出資料夾。
def save_json(data: Any, path: str | Path) -> None:
    """Write data as pretty UTF-8 JSON."""
...
# 讀取歷史問答資料池，檢查每筆資料是否含有檢索需要的欄位。
def load_documents(path: str | Path, limit: int | None = None) -> list[dict[str, Any]]:
    """Load document pool and validate the fields used by retrieval."""
...
# 讀取 query 檔案，檢查每筆 query 是否含有 ID 和 Question 欄位。
def load_queries(path: str | Path, limit: int | None = None) -> list[dict[str, Any]]:
    """Load query file and validate the fields used by retrieval."""
