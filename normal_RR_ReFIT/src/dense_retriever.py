...
class DenseRetriever:
    ...
    def encode_query(self, query: str) -> np.ndarray:
        ...
        prepared_query = format_query_with_instruction(normalize_text(query), self.query_instruction)
        return self._encode_texts([prepared_query])[0]
    ...
    def _encode_in_batches(self, texts: list[str], cache_paths: dict[str, Any]) -> np.ndarray:
        ...
        if self.use_cache and self._is_valid_partial_cache(partial_progress, cache_paths["cache_key"], len(texts)):
        ...
        for start in tqdm(
            ...
            if memmap is None:
                memmap = open_memmap(
                    cache_paths["partial_embeddings"],
                    mode="w+",
                    dtype=np.float32,
    ...
    def _cache_paths(self) -> dict[str, Any]:
        ...
        return {
            "cache_key": cache_key,
            "embeddings": self.cache_dir / f"qwen3_embedding_{cache_key}.npy",
            "metadata": self.cache_dir / f"qwen3_embedding_{cache_key}.json",
            "partial_embeddings": self.cache_dir / f"qwen3_embedding_{cache_key}.partial.npy",
            "partial_progress": self.cache_dir / f"qwen3_embedding_{cache_key}.partial.json",
        }

    def _cache_key(self) -> str:
        digest = hashlib.sha256()
        digest.update(self.model_name_or_path.encode("utf-8"))
        digest.update(str(self.max_length).encode("utf-8"))
        digest.update(str(self.use_4bit).encode("utf-8"))
        digest.update(self.compute_dtype.encode("utf-8"))
        digest.update(str(self.query_instruction).encode("utf-8"))
